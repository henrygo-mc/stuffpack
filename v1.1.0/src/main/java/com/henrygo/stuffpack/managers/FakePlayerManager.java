...
public class FakePlayerManager {
    ...
    private void startHideTabTask() {
        ...
        log("Started hide tab list task");
    }
    ...
    private void startLookTask() {
        ...
        log("Started look at player task");
    }
    ...
    public Player spawnFakePlayer(String name, Location location, String skinUrl) {
        ...
        try {
            ...
            log("Got MinecraftServer: " + minecraftServer.getClass().getName());

            Object serverLevel = getServerLevel(location);
            log("Got ServerLevel: " + serverLevel.getClass().getName());
            ...
            log("Set up channel for Connection");
            ...
            log("Created CommonListenerCookie");
            ...
            if (bukkitPlayer != null) {
                try {
                    ...
                    log("Teleported to final location (pitch=0)");
                } catch (Exception e) {
                    log("Teleport failed: " + e.getMessage());
                }
                
                bukkitPlayer.setOp(true);
                log("Granted OP to fake player");
                
                try {
                    bukkitPlayer.setGameMode(org.bukkit.GameMode.CREATIVE);
                    log("Set game mode to CREATIVE");
                } catch (Exception e) {
                    log("Failed to set game mode: " + e.getMessage());
                }
                
                try {
                    bukkitPlayer.setInvulnerable(true);
                    log("Set invulnerable");
                } catch (Exception e) {
                    log("Failed to set invulnerable: " + e.getMessage());
                }
                
                try {
                    setSkinViaPlayerProfile(bukkitPlayer, skinUrl);
                } catch (Exception e) {
                    log("Failed to set skin via PlayerProfile: " + e.getMessage());
                }
                ...
                Bukkit.getScheduler().runTaskLater(plugin, () -> {
                    hideFromTabList(name);
                    log("Hid fake player from tab list (delayed)");
                }, 20L);
    ...
    public void hideFromTabList(String name) {
        ...
        try {
            ...
            if (removePacket == null) {
                log("Remove packet is null, cannot hide from tab: " + name);
                return;
            }
            ...
            log("Hid fake player from tab list (packet only): " + name);
        } catch (Exception e) {
            log("Failed to hide from tab list: " + e.getMessage());
        }
    }
    ...
    public void hideAllFromPlayer(Player player) {
        for (Player fakePlayer : fakePlayers.values()) {
            ...
            try {
                ...
                log("Failed to hide fake player from " + player.getName() + ": " + e.getMessage());
            }
        }
    }
    ...
    private Object createPlayerInfoRemovePacket(Object entityPlayer) throws Exception {
        ...
        for (Class<?> c : playerInfoPacketClass.getDeclaredClasses()) {
            if (c.isEnum()) {
                for (Object constant : c.getEnumConstants()) {
                    if (constant.toString().contains("REMOVE_PLAYER")) {
                        actionClass = c;
                        break;
                    }
                }
        ...
        if (actionClass == null) {
            log("Could not find REMOVE_PLAYER action enum");
            return null;
        }
        
        Object removePlayerAction = null;
        for (Object constant : ((Enum[]) actionClass.getEnumConstants())) {
            if (constant.toString().equals("REMOVE_PLAYER")) {
                removePlayerAction = constant;
                break;
            }
        }
        
        if (removePlayerAction == null) {
            log("Could not find REMOVE_PLAYER action value");
            return null;
        }
        ...
        log("Could not create remove player info packet");
        return null;
    }
