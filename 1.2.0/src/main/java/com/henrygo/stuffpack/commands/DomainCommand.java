...
public class DomainCommand implements CommandExecutor {
    ...
    private boolean handleMove(Player player, String[] args) {
        if (args.length < 3) {
            player.sendMessage("§c用法: /dom move <领地ID/名称> <x偏移> <y偏移> <z偏移>");
            player.sendMessage("§c例如: /dom move 我的领地 x+1 y-1 z+0");
            return true;
        }
        ...
        try {
            ...
            domain = plugin.getDomainManager().getDomain(target);
            if (domain == null) {
                domain = findDomainByName(target);
            }
        }
        ...
        for (int i = 2; i < args.length; i++) {
            ...
            if (arg.startsWith("x")) {
                try {
                    dx = Integer.parseInt(arg.substring(1));
                } catch (NumberFormatException e) {
                    player.sendMessage("§c无效的x偏移: " + arg);
                    return true;
                }
            } else if (arg.startsWith("y")) {
                try {
                    dy = Integer.parseInt(arg.substring(1));
                } catch (NumberFormatException e) {
                    player.sendMessage("§c无效的y偏移: " + arg);
                    return true;
                }
            } else if (arg.startsWith("z")) {
                try {
                    dz = Integer.parseInt(arg.substring(1));
                } catch (NumberFormatException e) {
                    player.sendMessage("§c无效的z偏移: " + arg);
                    return true;
                }
            }
        }
        ...
        if (dx == 0 && dy == 0 && dz == 0) {
            player.sendMessage("§c偏移量不能全部为0");
            return true;
        }
        
        plugin.getDomainManager().moveDomain(domain, dx, dy, dz);
        plugin.getDomainManager().saveDomains();
        
        player.sendMessage("§a领地已移动!");
        player.sendMessage("§a偏移量: x" + (dx > 0 ? "+" : "") + dx + " y" + (dy > 0 ? "+" : "") + dy + " z" + (dz > 0 ? "+" : "") + dz);
        return true;
    }
}