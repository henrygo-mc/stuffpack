...
public class CoinCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length < 1) {
            sender.sendMessage("§c用法: /coin <give|take|pay|balance|sign> [player] [amount]");
            return true;
        }
        ...
        switch (action) {
            ...
            default:
                ...
                sender.sendMessage("§c可用操作: give, take, pay, balance, sign");
                return true;
        }
    }
    ...
    private boolean handleSign(CommandSender sender, String[] args) {
        ...
        if (!plugin.getLoginManager().isLoggedIn(player.getName())) {
            player.sendMessage("§c请先登录后再签到");
            return true;
        }
        
        if (!plugin.getCoinEconomy().canSign(player.getName())) {
            int todayCount = plugin.getCoinEconomy().getTodaySignCount(player.getName());
            int limit = plugin.getCoinEconomy().getDailySignLimit();
            player.sendMessage("§c今日已签到 " + todayCount + "/" + limit + " 次，明天再来吧！");
            return true;
        }
        
        long reward = plugin.getCoinEconomy().doSign(player.getName());
        if (reward == -1) {
            player.sendMessage("§c签到失败，请稍后再试");
            return true;
        }
        ...
        player.sendMessage("§7今日签到进度: " + todayCount + "/" + limit);
